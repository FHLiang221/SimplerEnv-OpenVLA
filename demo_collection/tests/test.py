...
_last = np.zeros(
    10, dtype=np.float32
)  # [action(7), plus flag, start flag, minus flag]
...
def get_switch_action() -> np.ndarray:
    ...
    if (
        abs(gripper_input) > 0.01
    ):  # threshold to detect button press (controller sends ±0.05)
        if gripper_input > 0:
...
def get_jaco_proprioception(env, obs) -> np.ndarray:
    ...
    return np.concatenate([pos, quat_xyzw, [gripper_state]], axis=0).astype(
        np.float32
    )
...
def collect_episode_data(
    env, env_name: str, episode_id: int, ignore_quit_for: int, base_dir=None
):
    ...
    print(
        "🎮 Start demonstrating with Jaco arm! (+ or - button to abort episode if needed)\n"
    )
    ...
    while True:
        ...
        if step_count > 0 and step_count % 5 == 0 and base_dir is not None:
            ...
            episode_frames_dir = (
                base_dir / "captured_frames" / f"episode_{episode_id:03d}"
            )
            ...
            render_frame_path = (
                render_frames_dir / f"step{step_count:03d}_{timestamp}.png"
            )
            cv2.imwrite(
                str(render_frame_path),
                cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR),
            )

            # Save training observation frame (224x224 - what gets saved for VLA training)
            training_frame_path = (
                training_frames_dir / f"step{step_count:03d}_{timestamp}.png"
            )
            cv2.imwrite(
            ...
            print(
                f"📸 Episode {episode_id} frames saved at step {step_count}: {timestamp}"
            )
        ...
        if np.any(np.abs(action) > 1e-3):
            ...
            if (
                "target_delta" in env.control_mode
                and "gripper" in env.control_mode
            ):
            ...
            jaco_proprio = get_jaco_proprioception(
                env, obs
            )  # 8D: EEF pose + gripper
...
def save_episode_to_file(episode_data, success_dir: Path, failure_dir: Path):
    ...
    episode_file = (
        output_dir
        / f"jaco_{status_prefix}_episode_{episode_data['episode_id']:06d}.npz"
    )
    ...
    images = np.stack(
        [s["image"] for s in steps], axis=0
    )  # [T, 224, 224, 3] uint8
    actions = np.stack([s["action"] for s in steps], axis=0).astype(
        np.float32
    )  # [T, 7]
    states = np.stack([s["state"] for s in steps], axis=0).astype(
        np.float32
    )  # [T, 8] (eef_pose+gripper)
    rewards = np.asarray([s["reward"] for s in steps], dtype=np.float32)  # [T]
    is_terminal = np.asarray(
        [s["is_terminal"] for s in steps], dtype=bool
    )  # [T]
    is_first = np.asarray([s["is_first"] for s in steps], dtype=bool)  # [T]

    # Debug: print shapes to verify correctness for Jaco
    print(
        f"Jaco Shapes: images={images.shape}, actions={actions.shape}, states={states.shape}, "
    ...
    print(
        f"💾 {status_emoji} JACO {status_text}: Episode {episode_data['episode_id']} saved to {episode_file}"
    )
    return episode_file
...
def collect_trajectory(env_name: str, num_trajs: int):
    ...
    dataset_name = (
        f"jaco_{env_name}_{num_trajs}trajs_switch_{timestamp}_all_episodes"
    )
    jaco_data_dir = Path("jaco_data")  # Relative to current directory
    jaco_data_dir.mkdir(
        parents=True, exist_ok=True
    )  # Create jaco_data dir if it doesn't exist
    ...
    camera_cfgs = {
        "base_camera": dict(
            p=pose.p, q=pose.q, width=128, height=128, fov=np.deg2rad(69.4)
        )
    }
    ...
    print(
        f"🤖 JACO ARM MODE: Action dim={JACO_ACTION_DIM}, Proprio dim={JACO_PROPRIO_DIM}"
    )
    print(
        "📝 ADAPTIVE MODE: Instructions captured per episode for randomized tasks"
    )
    print(
        "📸 FRAME CAPTURE MODE: PNG images saved every 5 steps with timestamps"
    )
    print(
    ...
    print(
        "Episode control: + = start episode / abort episode, - = abort episode"
    )
    ...
    while len(collected_successes) < num_trajs:
        ...
        episode_data = collect_episode_data(
            env, env_name, ep_i, ignore_quit_for, base_dir
        )
        ...
        if episode_data:
            ...
            if episode_data.get("success", False):
                # Success - always save
                episode_file = save_episode_to_file(
                    episode_data, success_dir, failure_dir
                )
                collected_successes.append(episode_file)
                print(
                    f"✅ Jaco Success {len(collected_successes)}/{num_trajs} collected"
                )
            else:
                ...
                if step_count == 0:
                    ...
                    print(
                        f"🗑️  Empty Jaco episode discarded automatically (still need {num_trajs - len(collected_successes)} successes)"
                    )
                else:
                    # Non-empty failure - prompt for decision
                    should_save = prompt_failure_decision(
                        episode_data, step_count
                    )
                    ...
                    if should_save:
                        ...
                        print(
                            f"💾 Jaco Failure {len(collected_failures)} saved (still need {num_trajs - len(collected_successes)} successes)"
                        )
                    else:
                        discarded_failures += 1
                        print(
                            f"🗑️  Jaco Failure {discarded_failures} discarded (still need {num_trajs - len(collected_successes)} successes)"
                        )
    ...
    metadata = {
        ...
        "total_saved_episodes": len(collected_successes)
        + len(collected_failures),
        ...
        "failure_save_rate": (
            len(collected_failures)
            / (len(collected_failures) + discarded_failures)
            if (len(collected_failures) + discarded_failures) > 0
            else 0
        ),
    ...
    failure_save_rate = (
        len(collected_failures) / total_failures if total_failures > 0 else 0
    )
